...
//                                                 ++++++++++++++++++++++++++++++++++++++++++++++++
   ...
   /*
       definition :         DOM = Document Object Model ,  dom is a programing interface jo browser provide karta hai.
...
/*                                                               COOKIES && WEBSTORAGE
                                                               ++++++++++++++++++++++++++++++++++
Cookies aur web storage dono browser me data store karne ke liye use hote hain, lekin inka purpose aur working alag hota hai. Cookies chhoti data files hoti hain jo website
user ke browser me store karti hai aur har HTTP request ke sath server ko bhejti hai. Iska use mainly login sessions, authentication, aur user identification ke liye hota 
hai. Cookies ka size limit lagbhag 4KB hota hai, aur inhe server aur client dono access kar sakte hain. Cookies me expiry date set ki ja sakti hai, jisse data automatic 
delete ho jata hai jab wo time complete ho jaye.

Wahin Web Storage, jisme localStorage aur sessionStorage aate hain, ye data ko browser me hi store karte hain aur server ke sath automatic share nahi karte. Web Storage ka
use mainly user preferences, theme settings, temporary data, ya cart information store karne ke liye hota hai. Iska size limit cookies se kaafi zyada hota hai — lagbhag 
5MB tak. localStorage tab tak data save rakhta hai jab tak manually delete na kiya jaye, jabki sessionStorage tab band hone par apna data khud delete kar deta hai.

Aasan shabdon me kahen to cookies un situations me use hoti hain jahan server ko data chahiye hota hai (like authentication), jabki web storage use hota hai jab data sirf 
browser me hi rakhna ho (like theme, username, cart data). Cookies har request ke sath server ko send hoti hain, jabki web storage ka data sirf client-side par hi rehta hai,
isliye ye zyada fast aur secure hoti hai.
...
Local Storage ek aisi storage jagah hoti hai jahan hum data permanently (tab tak jab tak manually delete na kiya jaye) store kar sakte hain.
Agar user browser band bhi kar de ya system restart kare, data fir bhi save rehta hai.
...
User ke theme preference save karna (dark/light mode)

Shopping cart data store karna

User ke settings ya token temporarily rakhna

🕓 2. Session Storage

Session Storage bhi localStorage jaisa hi hota hai, par iska data tab band hote hi delete ho jata hai.
Ye data temporary session ke liye store hota hai — jab tak tab/browser open hai.
...
🔹 Use case:
    ...
    OTP, temporary form data, ya current tab ke data ko store karna

    Aise data jise refresh ya close hone ke baad rakhne ki zarurat nahi hai



    ⚖️ Difference Between Local Storage & Session Storage (with 4 commands)
Command
	                                    Local Storage	                                                                        Session Storage

1. setItem()	Data permanently store hota hai (jab tak manually delete na karo).             	Data temporary store hota hai (tab/browser band hote hi delete ho jata hai).
2. getItem()	Data tab band karne ke baad bhi milta hai, kyunki wo save rehta hai.	          Data sirf tab tak milta hai jab tak current tab open hai.
3. removeItem()	Specific data ko delete karta hai, baki sab data rehta hai.	                  Same kaam karta hai, par session end hone par sab data automatically delete ho jata hai.
4. clear()	Poora localStorage ka data delete karta hai — permanently.	                      Poora sessionStorage ka data delete karta hai — sirf current tab ke liye.
*/
...
setTimeout aur setInterval dono JavaScript ke timing functions hain,jinse hum kisi code ko delay ke sath ya bar-bar repeat karwa sakte hain ⏱️

🔹 1. setTimeout()

👉 setTimeout() ka use kisi function ko ek baar run karne ke liye hota hai,
lekin thoda delay (specific time ke baad) ke sath.

Syntax:setTimeout(function, timeInMilliseconds);

➡️ Yahan code 3 second (3000ms) ke baad sirf ek baar chalega.

Use case:

Splash screen ya loader ke baad content show karna

Notifications ko kuch seconds baad dikhana

🔹 2. setInterval()

👉 setInterval() ka use kisi function ko bar-bar repeatedly run karne ke liye hota hai,
ek fixed interval (time gap) ke sath.
...
➡️ Yahan ye message har 2 second me repeat hota rahega.

Use case:

Digital clock banana

Real-time counter ya timer dikhana

Data ko har few seconds me refresh karna
...
/*                                                                Event in js && Event Object
                                                              ++++++++++++++++++++++++++++++++++

Js me event ak action hota he jo ki browser me user ke dwara hota he jo ki user ke click karne par , mouse move , mouse hover, mouse in ye sab karne par he jo  ki event 
trigger hote he js me ham eventListern ki help se ham in events ko capture karte he 
                                                     
1. Event Listener

"Event listener ek function hota hai jo browser me kisi element ke specific event ke occur hone par trigger hota hai aur code execute karta hai."
Example: element.addEventListener("click", function(){ ... })   

2.Event Object :
JavaScript me ek special object hai jo har event ke sath automatically pass hota hai. Isme event ke type, trigger element (target), aur mouse/keyboard details hoti hain. 
Hum ise use karke event ka context samajh sakte hain aur specific code execute kar sakte hain, jaise button click handle karna ya form submit rokna.
*/
...
"Event Delegation ek technique hai jisme hum parent element par event listener lagate hain aur uske child elements ke events ko handle karte hain."
...
Humne sirf parent <ul> par event listener lagaya.

Jab user kisi <li> par click karta hai, event bubble hoke <ul> tak aata hai.

Hum event.target se identify kar lete hain ki kaunsa child click hua.

Is tarah har <li> me listener lagane ki zarurat nahi padti.

✅ Benefit: Memory efficient aur dynamic elements ke liye useful.


🟡 2. Event Bubbling
🔹 Definition:

"Event Bubbling ek process hai jisme event inner element se start hota hai aur parent elements tak upar propagate karta hai."
...
document.getElementById("child").addEventListener("click", () => {
  alert("Child clicked");
   ...
   🔹 Explanation:
    ...
    Jab Child button click hota hai, pehle child ka event trigger hota hai,
    fir event bubble hoke parent element tak jata hai.
...
"Event Capturing ek process hai jisme event outer element se start hota hai aur inner element tak propagate karta hai."

<div id="outer" style="padding:20px; background:lightgreen;">
  Outer
  <div id="inner" style="padding:20px; background:lightcoral;">Inner</div>
</div>
...
🔹 Explanation:
    ...
    Jab inner div click hoti hai, capturing phase me event outer se start hota hai.

    Output order:
    1️⃣ Outer clicked (capturing)
    2️⃣ Inner clicked (target phase)
*/

/*                                                                How can you stop event propgation or event bubbling in js ?
                                        --------------------------------------------------------------------------------

🔹 Stopping Event Propagation / Event Bubbling in JavaScript

When an event occurs (like a click), it bubbles up from the child element to its parent elements in the DOM.
Agar tum chaaho ki event parent tak na jaye, to uske liye JavaScript me 2 methods hote hain 👇

🧩 1. stopPropagation()

Ye method event ko parent elements tak bubble hone se rok deta hai.
...
When you click the button, only
👉 Child clicked
message appears — Parent event doesn’t trigger because bubbling is stopped.


🧩 2. stopImmediatePropagation()

Ye method event bubbling ko rokne ke sath-sath same element par lagaye hue multiple listeners ko bhi trigger hone se rok deta hai.
...
btn.addEventListener("click", () => {
  alert("Second listener"); // ❌ won't run
});
</script>

🔹 Output:

Only "First listener" runs because the second one is stopped.


*/

/*                                                                ERROR Propgation in js 
                                                                      ++++++++++++++++++++++++++++
 Error Propagation ka matlab hai ek function me hone wali error ka upar ke calling functions tak pass hona,jab tak wo error kisi try...catch block me handle na ho jaye.
